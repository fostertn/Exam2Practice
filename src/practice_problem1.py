...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        self.volumeog = self.volume
        self.contentsog = self.contents
        self.historicalcontents = []
        """
        What comes in:
          -- self
    ...
    def shrink(self, new_volume):
        ...
        if self.volume < len(self.contents):
            for k in range(self.volume, len(self.contents)):
                container = container + self.contents[k]
            for k in range(self.volume):
                shrunk_string = shrunk_string + self.contents[k]
            self.contents = shrunk_string
        return container
    ...
    def double_then_shrink(self, new_volume):
        total = len(self.double()) + len(self.shrink(new_volume))
        return total
    ...
    def reset(self):
        self.historicalcontents.append(self.contents)
        self.cheatvolume = self.volume
        self.cheatcontents = self.contents
    ...
    def get_history(self):
        return self.historicalcontents
        """
        What comes in:
          -- self
        What goes out:
